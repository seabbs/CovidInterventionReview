...
## Extract variables of potential interest 

```{r extract-variables}
extracted_linelist <- linelist %>%
  dplyr::as_tibble() %>%
...
saveRDS(tab_cases_imported, "cases_imported.rds")
...
wuhan <- readr::read_csv("wuhan_20200303.csv") %>% 
...
```{r}
interventions <- readr::read_csv("intervention_dates.csv") %>% 
...
saveRDS(summarise_interventions, "intervention_freq.rds")
...
saveRDS(social_interventions, "social_interventions.rds")
...
saveRDS(non_social_interventions, "non_social_interventions.rds")
